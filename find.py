...
if __name__ == '__main__':
    ...
    # Verificare pentru numarul si tipul argumentelor
    if len(sys.argv) >= 2:
        # Daca primul argument este un director
        if '/' in sys.argv[1]:
            directory = sys.argv[1]

        # Daca primul argument este o extensie
        elif sys.argv[1].startswith('.'):
            extension = sys.argv[1]

        # Daca primul argument este numele unui fisier
        elif not sys.argv[1].startswith('-'):
            file_name = sys.argv[1]

    if len(sys.argv) >= 3:
        # Daca al doilea argument este -iname sau -name
        if sys.argv[2] == '-iname':
            case_insensitive = True
        elif sys.argv[2] == '-name':
            case_insensitive = False
        # Daca al doilea argument este o extensie
        elif sys.argv[2].startswith('.'):
            extension = sys.argv[2]
        # Daca al doilea argument este numele unui fisier
        else:
            file_name = sys.argv[2]

    if len(sys.argv) >= 4:
        # Daca al treilea argument este o extensie
